...
fn run_e1d_cmd(args: &[String]) -> Result<(), String> {
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "--genesis-cooldown-steps" => {
                let value = take_value(args, i)?;
                cfg.genesis_cooldown_steps = parse_u64(value, "genesis-cooldown-steps")?;
                i += 2;
            }
            "--genesis-margin-threshold" => {
                let value = take_value(args, i)?;
                cfg.genesis_coverage_margin_threshold = value.parse::<u32>()
                    .map_err(|_| format!("invalid genesis-margin-threshold '{value}'"))?;
                i += 2;
            }
            "--genesis-disagreement-rate" => {
                let value = take_value(args, i)?;
                cfg.genesis_disagreement_rate_threshold = value.parse::<f32>()
                    .map_err(|_| format!("invalid genesis-disagreement-rate '{value}'"))?;
                i += 2;
            }
            other => return Err(format!("unknown argument '{other}'")),
        }
